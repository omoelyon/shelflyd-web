...
function StorefrontCheckoutContent({ slug }: { slug: string }) {
  ...
  const { data: locations } = useQuery({
    queryKey: ['storefront', slug, 'delivery-locations'],
    queryFn: () => storefrontApi.getDeliveryLocations(slug),
    enabled: !!slug,
  });
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-10 space-y-6">
      ...
      <form onSubmit={handleSubmit((d) => checkoutMutation.mutate(d))} className="space-y-6">
        {/* Order type */}
        <Card>
          <CardHeader>
...
export default function StorefrontCheckoutPage({ params }: Props) {
  const { slug } = use(params);
  return (
    <Suspense fallback={<div className="max-w-2xl mx-auto px-4 py-10"><Skeleton className="h-96 rounded-xl" /></div>}>
      <StorefrontCheckoutContent slug={slug} />
    </Suspense>
  );
}
